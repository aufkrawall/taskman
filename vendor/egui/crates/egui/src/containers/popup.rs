...
impl<'a> Popup<'a> {
    ...
    pub fn show<R>(self, content: impl FnOnce(&mut Ui) -> R) -> Option<InnerResponse<R>> {
        ...
        // TASKMAN-FORK: popups are separate layers, but they can be built while a
        // ScrollArea behind them is still inside its content closure. A scroll-to
        // request generated by popup focus/navigation would otherwise be consumed by
        // that ScrollArea and make the underlying list jump. Preserve any target that
        // belonged to the surrounding UI and discard unhandled popup-local targets.
        let saved_scroll_target =
            ctx.pass_state_mut(|state| std::mem::take(&mut state.scroll_target));
        ...
        ctx.pass_state_mut(|state| state.scroll_target = saved_scroll_target);
