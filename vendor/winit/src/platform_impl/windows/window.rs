...
use windows_sys::Win32::Foundation::{
    HWND, HINSTANCE, LPARAM, OLE_E_WRONGCOMPOBJ, POINT, POINTS, RECT, RPC_E_CHANGED_MODE, S_OK,
    WPARAM,
};
...
use windows_sys::Win32::UI::WindowsAndMessaging::{
    ...
    GWLP_HINSTANCE, HMENU, HTBOTTOM, HTBOTTOMLEFT, HTBOTTOMRIGHT, HTCAPTION, HTLEFT, HTRIGHT,
    ...
    WDA_EXCLUDEFROMCAPTURE, WDA_NONE, WINDOW_EX_STYLE, WINDOW_STYLE, WM_NCLBUTTONDOWN,
...
/// TaskMan fork: create the window in a specific window band when the
/// `TASKMAN_WINDOW_BAND` environment variable parses to a non-zero band.
///
/// Native Windows Task Manager creates its main window with
/// `CreateWindowInBand` in band 16, which is above the Start menu's band 6;
/// that is why its "Always on top" stays above the shell. `SetWindowBand`
/// cannot move an existing window into band 16 (`ERROR_ACCESS_DENIED`), so
/// the band must be chosen at creation. With the variable unset (or 0), this
/// is exactly the stock `CreateWindowExW` path. `CreateWindowInBand` is not
/// in the user32 import library, so it is resolved dynamically, like Task
/// Manager itself does.
unsafe fn create_window_native(
    ...
    let band = std::env::var("TASKMAN_WINDOW_BAND")
        .ok()
    ...
    if let Some(band) = band {
        ...
        let user32 = unsafe { GetModuleHandleW(windows_sys::w!("user32.dll")) };
        if user32 != 0 {
            if let Some(proc) =
                unsafe { GetProcAddress(user32, windows_sys::s!("CreateWindowInBand")) }
            {
                let create: CreateWindowInBand = unsafe { core::mem::transmute(proc) };
                return unsafe {
                    create(
                        ex_style,
                        class_name,
