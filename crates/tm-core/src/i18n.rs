...
keys! {
    ...
    ProcessTreeView => ["Prozessstruktur (Namensspalte)", "Process tree (Name column)"],
    ...
    FileProperties => ["Dateieigenschaften", "File properties"],
    ProcessProperties => ["Prozesseigenschaften", "Process properties"],
    ...
    PropDiskWrite => ["Datenträger schreiben", "Disk write"],
    PropIoReadTotal => ["Gelesen gesamt", "Total read"],
    PropIoWriteTotal => ["Geschrieben gesamt", "Total written"],
    PropNetworkReceive => ["Netzwerk empfangen", "Network receive"],
    PropNetworkSend => ["Netzwerk senden", "Network send"],
    PropNetworkReceivedTotal => ["Empfangen gesamt", "Total received"],
    PropNetworkSentTotal => ["Gesendet gesamt", "Total sent"],
    ...
    SecurityUnavailable => ["Sicherheitsinformationen nicht verfügbar", "Security information unavailable"],
    PropImageType => ["Abbildtyp", "Image type"],
    PropProtection => ["Schutz", "Protection"],
    PropCriticalProcess => ["Kritischer Prozess", "Critical process"],
    PropMitigationPolicies => ["Mitigationsrichtlinien", "Mitigation policies"],
    MitDep => ["Datenausführungsverhinderung (DEP)", "Data Execution Prevention (DEP)"],
    MitAslrBottomUp => ["ASLR: Bottom-up", "ASLR: bottom-up"],
    MitAslrForceRelocate => ["ASLR: Abbilder erzwingend verschieben", "ASLR: force image relocation"],
    MitAslrHighEntropy => ["ASLR: hohe Entropie", "ASLR: high entropy"],
    MitDynamicCode => ["Dynamischen Code verbieten", "Prohibit dynamic code"],
    MitStrictHandles => ["Strikte Handle-Prüfungen", "Strict handle checks"],
    MitExtensionPoints => ["Erweiterungspunkte deaktivieren", "Disable extension points"],
    MitCfg => ["Kontrollflusswächter (CFG)", "Control Flow Guard (CFG)"],
    MitCfgStrict => ["CFG: strikter Modus", "CFG: strict mode"],
    MitWin32k => ["Win32k-Systemaufrufe deaktivieren", "Disable Win32k system calls"],
    MitMicrosoftSigned => ["Nur von Microsoft signierte Abbilder", "Microsoft-signed images only"],
    MitStoreSigned => ["Nur Store-signierte Abbilder", "Store-signed images only"],
    MitFonts => ["Nicht-Systemschriftarten blockieren", "Block non-system fonts"],
    MitRemoteImages => ["Remote-Abbilder blockieren", "Block remote images"],
    MitLowLabelImages => ["Low-Integrity-Abbilder blockieren", "Block low-integrity images"],
    MitPreferSystem32 => ["System32-Abbilder bevorzugen", "Prefer System32 images"],
    MitChildProcesses => ["Untergeordnete Prozesse blockieren", "Block child-process creation"],
    MitCetShadowStack => ["CET-Benutzerschattenstapel", "CET user shadow stacks"],
    MitCetStrict => ["CET: strikter Modus", "CET: strict mode"],
    MitSehop => ["SEHOP", "SEHOP"],
    MitSideChannel => ["Seitenkanalisolierung", "Side-channel isolation"],
    MitPayload => ["Exploit-Nutzlastbeschränkungen", "Exploit payload restrictions"],
    ...
    MemModified => ["Geändert", "Modified"],
    MemStandby => ["Bereitschaft", "Standby"],
    MemFree => ["Frei", "Free"],
    ...
    DumpCancelled => ["Erstellung des Speicherabbilds abgebrochen", "Dump creation canceled"],
    DumpCancelling => ["Wird abgebrochen...", "Canceling..."],
    DumpCreating => ["Speicherabbild wird erstellt...", "Creating dump file..."],
    DumpElapsed => ["Verstrichene Zeit:", "Elapsed:"],
