...
pub fn show(app: &mut TaskManApp, ui: &mut egui::Ui) {
    ...
    ui.label(format!("Gesamte CPU-Zeit aller Apps: {}", tm_core::format::format_cpu_time(total_cpu)));
    ui.separator();

    let mut rows = entries;
    rows.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));

    egui::ScrollArea::vertical().id_salt("apphistory").show(ui, |ui| {
        ...
        egui::Grid::new("apphist-rows")
            ...
            .show(ui, |ui| {
                for (name, cpu_s, net_b) in rows.iter().take(500) {
                    ui.monospace(name);
                    ui.label(format!("{}  ({:.1} %)", tm_core::format::format_cpu_time(*cpu_s), pct_of(*cpu_s, total_cpu)));
