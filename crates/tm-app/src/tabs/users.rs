...
pub fn show(app: &mut TaskManApp, ui: &mut egui::Ui) {
    ...
    egui::ScrollArea::vertical().id_salt("users").show(ui, |ui| {
        for s in sessions {
            ...
            if let Some(ref snap) = snap {
                ...
                for p in &snap.processes {
                    if let Some(ref u) = p.user {
                        if u.eq_ignore_ascii_case(&s.user)
                            || (!name.is_empty() && !name.starts_with("session") && u.eq_ignore_ascii_case(name))
                        {
                            cpu += p.cpu_pct;
                            mem += p.mem_bytes;
                            count += 1;
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            stat(ui, &pal, "Arbeitsspeicher", &tm_core::format::format_bytes(mem));
                            ...
                            #[cfg(target_os = "windows")]
                            {
                                ...
                                if ui.button("Abmelden").clicked() {
                                    match app
                                        .actions
                                        .control_user_session(s.id, UserSessionAction::Logoff)
                                    {
                                ...
                                if ui.button("Trennen").clicked() {
                                    match app
                                        .actions
                                        .control_user_session(s.id, UserSessionAction::Disconnect)
                                    {
...
#[cfg(target_os = "windows")]
fn guard_lock(_app: &TaskManApp) -> PlMutexGuardStub {
    PlMutexGuardStub
}
#[cfg(target_os = "windows")]
struct PlMutexGuardStub;
