...
pub fn show(app: &mut TaskManApp, ui: &mut egui::Ui) {
    ...
    egui::ScrollArea::both().id_salt("startup-table").show(ui, |ui| {
                        ...
                        .allocate_ui(egui::vec2(280.0, 22.0), |ui| {
                            ui.vertical_centered_justified(|ui| {
                                ui.add_sized(
                                    [270.0, 18.0],
                                    egui::Label::new(
                                        egui::RichText::new(&item.name).size(12.5).color(pal.text),
                                    )
                                    .truncate(),
                                );
                            });
...
{}", item.command, item.location));
                    ...
                    resp.context_menu(|ui| {
                        ...
                        if ui.button(if item.enabled { "Deaktivieren" } else { "Aktivieren" }).clicked() {
                            let new_enabled = !item.enabled;
                            match app.actions.set_startup_enabled(&item.id, &item.location, new_enabled) {
                                Ok(()) => {
...
fn show_command_tooltip(ui: &egui::Ui, item: &tm_core::model::StartupItem) {
    if item.command.chars().count() > 40 || item.publisher.is_some() {
        resp_on_hover(ui).on_hover_text(format!("{}\n{}", item.command, item.location));
    }
}

fn resp_on_hover(_ui: &egui::Ui) -> egui::Response {
    // Placeholder replaced by direct tooltip on the label response in practice.
    unreachable!()
}
...
fn status_text(ui: &mut egui::Ui, enabled: bool) {
    ...
    ui.horizontal(|ui| {
        ...
        ui.painter().circle_filled(rect.center(), 3.0, if enabled { pal.ok_green } else { pal.text_dim });
