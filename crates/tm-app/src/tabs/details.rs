    ...
    let resp = match col {
        DetailColumn::Name => {
            ...
            if selected {
                ...
                ui.painter()
                    .rect_filled(rect, 3.0, pal.accent.gamma_multiply(0.15));
            }
            r
        }
        DetailColumn::Pid => weak(p.pid.to_string()),
        DetailColumn::Status => weak(match p.status {
        ...
        DetailColumn::Cpu => {
            let intensity = if max_cpu > 0.001 {
                (p.cpu_pct / max_cpu).sqrt()
            } else {
                0.0
            };
            heat_text(
        ...
        DetailColumn::CpuTime => weak(
            p.cpu_time_s
                .map(format::format_cpu_time)
                .unwrap_or_default(),
        ),
        DetailColumn::Memory => {
            let intensity = if max_mem > 0 {
                (p.mem_bytes as f32 / max_mem as f32).sqrt()
            } else {
                0.0
            };
            heat_text(
        ...
        DetailColumn::Commit => weak(p.commit_bytes.map(format::format_bytes).unwrap_or_default()),
        DetailColumn::Peak => weak(
            p.peak_mem_bytes
...
fn heat_text(ui: &mut egui::Ui, pal: &theme::Palette, t: f32, s: String, w: f32) -> egui::Response {
    ...
    if t > 0.02 {
        ui.painter()
            .rect_filled(rect, 3.0, theme::heat_color(pal, t.clamp(0.0, 1.0)));
    }
    ui.painter().text(
...
fn sort_rows(v: &mut [&ProcessEntry], col_idx: usize, asc: bool) {
    ...
    v.sort_by(|a, b| {
        let o: Ordering = match col {
            ...
            DetailColumn::CpuTime => a
                .cpu_time_s
                .partial_cmp(&b.cpu_time_s)
