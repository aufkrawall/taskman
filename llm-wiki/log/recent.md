...
- 2026-09-21: Diagnostics pass. The app had a complete logging subsystem that produced exactly one line per run, and no crash record at all; both are fixed, plus four smaller findings from the same review.
  ...
  - **`DeferredFileLayer` carried no filter.** It was added to the registry unfiltered, which makes every level interesting to the whole subscriber — defeating `tracing`'s global max-level short-circuit on the sampler's hot path, and burying our records under wgpu/naga trace spam the moment a file sink existed. It now clones `init_early`'s filter.
  - **A release crash left nothing behind.** `quiet_panic_hook` logged through the sink above and captured no backtrace; release is `panic = "abort"`, so the hook returns into `abort()`, `NonBlocking::flush` is a documented no-op and the worker thread is never scheduled again. `write_crash_record` now appends synchronously to `<log>.crash` with `sync_data`: payload, thread, `Backtrace::force_capture`, and the pre-attach ring when the file sink never came up (the startup-crash case, where the ring is the only history). Panic-hook safe throughout — `try_lock` not `lock`, because the panicking thread may already hold either mutex in `DeferredFileLayer::on_event` and blocking there hangs the process instead of aborting it. Release `strip` moved `symbols` → `debuginfo` so those backtraces symbolize; measured cost 52 KB on a 14.8 MB binary, no runtime change.
  - **A crash orphaned its ETW sessions.** An ETW session is a kernel object that outlives its host and keeps tracing every disk and network operation on the machine; teardown was `Drop`-only, which `panic = "abort"` and a kill both skip. Fixed session names meant the NEXT run reclaimed the orphan, but a user who crashes and does not relaunch keeps paying until reboot. `etw::stop_live_sessions` is now registered through the new `tm_core::logging::set_panic_cleanup` the first time a session starts, and runs BEFORE the crash record. It stops only sessions THIS process started (lock-free `LIVE_SESSIONS` flags over the exhaustive `SESSION_NAMES`), so a crashing GUI cannot stop the still-healthy service's. `net_etw`/`disk_etw` each pin their names against that list.
  - **`build.py --live-tests`** runs the five `#[ignore]`d tests and exits; Windows-only, and it refuses unelevated rather than reporting a pass it did not earn. They are the only checks that prove the hand-written NT offsets and ETW decoders against a live kernel, and `--check` cannot run them. `memory_list_layout_decodes_plausible_page_counts` passes unelevated on this build; the disk/network ones still need an elevated run (`known-debt.md` still records the 2026-09-11 system-logger switch as unproven on live events).
  - **`sample_inner` 760 → 629 lines.** `collect_gpus`/`collect_disks`/`collect_networks` extracted verbatim (diffed line-for-line), `apply_process_gpu` split out, and the zombie-eviction rule extracted as `is_zombie` with seven unit tests. Its `terminated`/`live_in_kernel_table` arguments are CLOSURES, not `bool`s: `is_process_terminated` opens a process handle, so eager evaluation would put one `OpenProcess` per process per tick on the hot path. The process loop itself stays inline.
  - **`startup.rs` had 541 lines and no tests; it now has seven, and one real bug is fixed.** `approval_says_enabled` reads a `StartupApproved` value: an ABSENT value meant enabled, but a present-but-EMPTY one meant disabled, because `data.first()` on an empty slice is `None` and `is_some_and` then yields false. Nothing in an empty value says "disabled" and Windows runs the item at the next logon, so the Startup page greyed out an entry that was live — the same class of error as rendering missing telemetry as zero. Both open-coded copies of the rule are now one function; `disabled_approval_blob` is the matching encoder.
  - Audited and found sound, so it is not re-derived: the `WH_KEYBOARD_LL` callback (one `u32` compare for any non-Escape key, then `CallNextHookEx`; no allocation, no locks, the expensive work punted to a worker via `SetEvent`), `force_foreground`'s hung-check and symmetric detach, the hot sampler path asking only for `PROCESS_QUERY_LIMITED_INFORMATION`, no `timeBeginPeriod` anywhere (the global timer resolution is never raised), atomic settings writes (temp + rename), and the hidden-surface background contract. Two accepted residuals recorded in `known-debt.md` § System state this program can leave behind: the IFEO registration outliving a deleted app, and sysinfo's own `PROCESS_VM_READ` handle per process.
  - Note for the next patch author: a Python patch script written through a bash heredoc silently ate a backslash and wrote a literal NUL byte into `etw.rs` (valid Rust inside a char literal, compiles and tests clean, but git then treats the file as binary). Scan touched files for NULs after scripted edits.
- 2026-09-20: Released v0.1.13 (tag on `46cdfd6`, the bump commit). Followed `build.md` §Publishing path: `--check` + `--audit` green, stable clippy green, bump pushed on main, `build.py --all-targets` for all four archives (Windows x86_64/ARM64, Linux x86_64/ARM64), `sha256` beside each, `gh release create --latest` with the full sha as `--target`. Release covers: VidMm-accurate dedicated and shared GPU memory reporting via `\GPU Adapter Memory(*)\Dedicated Usage` & `Shared Usage` performance counters, zero-overhead background operation with visibility-gated telemetry demand and stretched cadence, dynamic event-driven WH_KEYBOARD_LL keyboard hook installation via IFEO RegNotifyChangeKeyValue monitoring, topmost keeper cleanup without EVENT_OBJECT_REORDER, hung window protection during foreground activation, and ephemeral trace watchdog lifecycle management. Verified packaged archives: `taskman.exe` and `taskman-service.exe` in Windows zip pass `--selfcheck` (`--mock`), Linux binary passes `--selfcheck` under WSL.
- 2026-09-19: Fixed heavily overstated dedicated GPU memory reporting on Windows:
  - **Root cause:** `gpu::merge()` previously computed adapter `dedicated_used_bytes` and `shared_used_bytes` by summing `\GPU Process Memory(*)\Dedicated Usage` across all processes matching the adapter's LUID. Under WDDM, `\GPU Process Memory` represents per-process virtual committed video memory. Furthermore, display surfaces and window buffers are shared across processes and Desktop Window Manager (`dwm.exe`). Summing all process instances double-counted shared surfaces (e.g. `dwm.exe` alone routinely reports 8-10+ GB of virtual committed dedicated memory due to composed windows), causing an idle adapter using ~1.5 GB of VRAM to be reported and charted at 11+ GB (often 90-100% capacity) on the Performance GPU page.
  - **Fix:** Added `\GPU Adapter Memory(*)\Dedicated Usage` and `\GPU Adapter Memory(*)\Shared Usage` to the GPU PDH query group in `tm-platform::win::perfcounters`, returning typed `GpuAdapterMemRecord`s directly from the Windows Video Memory Manager (VidMm). Updated `gpu::merge()` to read the adapter's dedicated and shared memory directly from `GpuAdapterMemRecord` by LUID matching instead of summing process instances. Per-process memory in `process_gpu_view` continues using `GpuMemRecord` from `\GPU Process Memory` for the Details tab columns.
  - **Verification:** Unit and live counter tests in `tm-platform` verify that `read_gpu_adapter_memory` warms up and samples sane VRAM numbers matching Windows Task Manager.
- 2026-09-19: Background-impact audit and fixes. The question was whether anything this program does can degrade the rest of the machine while it sits in the tray; four things could, all now closed. See `current.md` § Background-cost contract for the resulting rules.
  - **Telemetry demand ignored visibility.** `demand_for` derived everything from the active tab, so "start with Windows + close to tray + default page Processes" kept both kernel ETW sessions (`Microsoft-Windows-Kernel-Network` all keywords; `SystemIoProviderGuid` disk + `SystemProcessProviderGuid` thread rundown) running for the whole logon session. Measured on the dev box before the fix: `TaskMan-Net-Service` 17354 and `TaskMan-Disk-Service` 27185 buffers of 64 KB in 37 minutes, i.e. ~1.25 MB/s of kernel trace throughput for a tray icon. A hidden surface now demands `TelemetryDemand::hidden()` (`CORE_PROCESS` only). Visibility is detected as "eframe ran `ui` last frame" — `update_logic_only` skips the UI pass for a hidden, minimized OR occluded window while still running `logic`, so one flag covers all three without plumbing shell state down from `NativeApp`.
  - **The tick and the thread priority ignored it too.** Hidden now means `HIDDEN_INTERVAL` (5 s) instead of the user's speed, plus `THREAD_MODE_BACKGROUND_BEGIN` on the engine thread via the new `SystemCollector::set_background`. Stretching the tick is safe because the charts are timestamp-anchored (`chart::x_on_axis`) and `AppHistoryDb::observe` already scales by the engine interval; restoring the window forces one immediate sample. The process priority class stays `ABOVE_NORMAL` deliberately — lowering it would weaken the one case a task manager exists for.
  - **`WH_KEYBOARD_LL` was installed unconditionally**, so every keystroke on the desktop was routed through this process even for a user who never enabled the Task Manager replacement, and the callback did `current_exe()` + an HKLM read (2 s cache) on the input path. The hook is now installed only while the IFEO registration is present and removed when it goes, driven by `RegNotifyChangeKeyValue` on the IFEO root (`taskmgr_replacement::ReplacementWatch`; the ROOT, because the `taskmgr.exe` subkey disappears with the registration and a notification armed on it would be lost). `tm-hotkey-hook` keeps the registration and handles `WM_SET_HOOK`; the callback reads one atomic. Worker waits moved from a 500 ms `WaitForSingleObject` poll to `MsgWaitForMultipleObjects` on [trigger, registry-change] with `INFINITE`.
  - **`force_foreground` attached input queues from a thread with no message pump.** `AttachThreadInput` merges two input queues, and a non-pumping partner is how the other application stops processing input; the activation calls can also block on the other side's pump. `tm-hotkey-worker` now pumps (`drain_messages`), and a foreground window that is already hung is never attached to — activation still runs, just without the foreground-lock bypass.
  - **Three system-wide `SetWinEventHook`s, never removed.** `EVENT_OBJECT_REORDER` is gone (it fires for z-order churn inside other processes' list/tree/tab controls, each one a cross-process marshal into this process before the callback could reject it); the remaining foreground/show hooks are stored and unhooked when always-on-top is switched off, instead of living behind a `Once` for the rest of the session.
  - **Broker trace watchdogs outlived their traces.** `spawn_disk_trace_watchdog`/`spawn_net_trace_watchdog` were `OnceLock`-guarded threads polling every 5 s forever; they share `spawn_trace_watchdog` now, which sleeps to the exact idle deadline and returns once it has stopped the session.
  - Measured and deliberately NOT changed, so it is not re-investigated: `DwmGetWindowAttribute(DWMWA_CLOAKED)` is ~20 ns/call (shared section, no `dwm.exe` RPC), so the per-window-per-tick call in `windows_enum` is free; a keyword mask on `Microsoft-Windows-Kernel-Network` is a no-op because all 22 of its events carry IPv4 (0x10) or IPv6 (0x20) — only an `EVENT_FILTER_TYPE_EVENT_ID` filter would narrow it to the eight data events, and that has no automated coverage; PDH is not the per-tick cost (`GPU Engine(*)` = 0.35 ms over 522 instances, `PhysicalDisk(*)` = 0.06 ms), core process enumeration is.
  - Not done: the core service stays `AutoStart`. `DemandStart` would need an unelevated GUI to start a service, which is exactly the privilege boundary the broker exists to avoid crossing.
- 2026-09-19: Released v0.1.12 (tag on `e9b1fdf`, the bump commit). Followed `build.md` §Publishing path: `--check` + `--audit` green, stable clippy green, bump pushed on main, `build.py --all-targets` for all four archives (Windows x86_64/ARM64, Linux x86_64/ARM64), `sha256` beside each, `gh release create --latest` with the full sha as `--target`. Release covers: authoritative kernel table zombie eviction without sampler freeze, cycle-safe PPID hierarchy walking, false-positive elimination for "Not responding" status on background games, restored action toast TTL with a "Clear all" stack button, low-latency WH_KEYBOARD_LL hotkey hook handling, and truthful GPU driver version reporting. Verified packaged archives: `taskman.exe` and `taskman-service.exe` in Windows zip, selfcheck verified under Windows and WSL.
- 2026-09-19: Toast lifetime restored, and "Not responding" no longer fires on throttled background games:
  - Action toasts expire again after `TOAST_TTL` (30 s), fading over the last `TOAST_FADE` (600 ms) so one never vanishes mid-glance. 8a36e5b had removed the TTL outright to stop accidental click-dismissal; the close button (the part that fix was actually about) stays, and the expiry is back on top of it. `TaskManApp::update` schedules the repaint that performs the expiry — nothing else wakes an idle app — switching to a 33 ms cadence only once a toast is inside its fade.
  - A stack of more than one toast now carries a "Clear all" button (`app_ui::original::clear_all_button`, `K::ClearAllToasts`) above the newest toast, where it cannot be hit while aiming for a close button. Test note: the FIRST frame of a new egui `Area` lays it out before its anchor applies, so `Memory::area_rect` only reports the resting position from the frame after — a click test must render twice before reading the rect.
  - Fixed the false "Not responding" on a game running at low fps in borderless windowed mode in the background. `IsHungAppWindow` is an EDGE trigger — true the moment a window's thread has not pumped for 5 s, false again the moment it does — so any frame interval above 5 s trips it even though the process is rendering and answers the next click. `tm-platform::win::sampler::NotRespondingHold` now requires the hung state to hold CONTINUOUSLY for `NOT_RESPONDING_GRACE` (10 s) before the status is set: one pumped tick clears the hold, so no frame interval below ~15 s can be reported, while a genuinely wedged process never clears it. The hold is keyed by pid + creation time (a recycled pid starts its own) and pruned to pids that own a window. `windows_enum::record` keeps reporting the raw edge and says so; the sampler is the single choke point every consumer of `ProcessEntry::status` reads through.
  - Open, if the false positive survives: the next corroboration would be per-process GPU activity (evidence the process is still presenting). Deliberately not added yet — it depends on GPU telemetry being demanded, and liveness-by-progress risks the opposite error on a process wedged in a spin loop.
- 2026-09-19: Review pass over the v0.1.10..v0.1.11 work; five defects fixed before shipping:
  - `CpuLoadAccountant::is_process_live` judged liveness from a table that may be STALE. `sample()` leaves `prev` untouched when `NtQuerySystemInformation` fails, so after a failure lasting longer than the 2 s new-process grace every newly started process was absent from it and got evicted from the snapshot — processes silently missing from a task manager. Absence is now evidence only while the table is younger than `PROC_TABLE_MAX_AGE` (5 s).
  - Zombie eviction hid the symptom but left the cause: sysinfo's own `PROCESS_VM_READ` handle is part of what pins a crashed process's `EPROCESS`, and omitting the row does not release it. `should_release_handles` (sampler.rs) recreates `System` again — but only on the first sighting of a given zombie (`Sampler::zombie_pids`) and at most once per `SYS_RESET_COOLDOWN` (15 s), which is exactly what the per-tick version lacked.
  - `hotkey_hook`: `Drop` tore down the worker event BEFORE unhooking, so a callback that had already read `TRIGGER_EVENT` could `SetEvent` a closed — and by then recycled — handle. The hook is uninstalled first now, and `stop_worker` is shared with the two `install` failure paths that previously leaked the event and left `WORKER_RUNNING` set. The worker also captures the handle value instead of re-reading the global it would find cleared.
  - `gpu::adapters` formatted a zero `CheckInterfaceSupport` result as `0.0.0.0`; an adapter with no user-mode driver now reports an empty string, which the Performance page renders as unavailable (core invariant: never a fabricated value).
  - `startup::toggle_item` looked `location` up from the startup cache ON THE WORKER THREAD — a lock both callers hold for the whole frame, with an empty-string fallback if a refresh had replaced the row. Captured at dispatch instead; the failure rollback still runs on the worker.
  - Reviewed and found sound: PPID cycle guards (all four walks), `terminate_handle`'s `GetExitCodeProcess` fallback, `SeDebugPrivilege` activation, the single-instance fast path (CLI helper modes all exit before it; `activate_existing` marks `PROBE_UNRESPONSIVE` so the second probe short-circuits), `StartupImpactTracker`, the dialog key helpers, and the memory composition bar's in-use derivation.
  - Open UX question raised here, resolved 2026-09-19: action toasts no longer expired at all (8a36e5b). `MAX_TOASTS` bounded them at 6, but an ignored toast occupied the corner for the rest of the session; a 30 s TTL and a "Clear all" button now answer it.
- 2026-09-18: Ghost/zombie process eviction, PPID cycle detection, and sampling freeze fix:
  - Eliminated sampler 100% CPU lockup and process page freeze caused by `self.sys = System::new()`: recreating `sysinfo::System` inside the sampling loop when dead PIDs were found forced a full system process re-enumeration on every tick. Because external handles (from Steam, Discord, RTSS, Defender, or WER) linger on exiting/zombie processes, Toolhelp continued enumerating them, making `dead_pids` non-empty on every single sample. This caused continuous LSASS SID lookups (`LookupAccountSidW`) and process memory reads across all 300+ processes, starving the engine thread (`tm-engine`) in heap teardown and freezing the UI. Removed the per-tick `System::new()`. (Superseded 2026-09-19: the recreation is back, but gated on the FIRST sighting of a given zombie and a 15 s cooldown, so the per-tick storm cannot recur.)
  - Eliminated infinite loop in `refine_categories_and_group_apps`: attached debugger (`cdb.exe`) to running frozen TaskMan instance and inspected `tm-engine` thread, finding RIP stuck at `taskman+0x72338d` in the app-root ancestor walk (`loop { ... cur = pi; }`). On Windows, Toolhelp parent PIDs can be cyclic due to PID reuse or parent spoofing (`PROC_THREAD_ATTRIBUTE_PARENT_PROCESS`). When a windowed process or game (e.g. Alan Wake 2) exited and its window was destroyed, walking upward through windowless non-core background ancestors entered cyclic PPID loops with zero cycle detection, spinning at 100% CPU indefinitely and preventing any snapshots from being published. Added `seen_in_walk: HashSet<usize>` to break cycles and exported canonical `is_plausible_parent` (`tm_core::model::is_plausible_parent`) to reject traversals into reused PIDs from the future across `sampler.rs`, `processes.rs`, and `details.rs`.
  - Added authoritative NT kernel process table liveness check (`CpuLoadAccountant::is_process_live` in `tm-platform::win::cpu_load`) using `NtQuerySystemInformation(SystemProcessInformation)`, the exact source table queried by native Windows Task Manager. Tolerates 1-second timestamp skew while rejecting PID reuse.
  - Fixed zombie game processes (e.g. Alan Wake 2 stuck in driver/GPU rundown) lingering in Details: previously, eviction was gated on `threads <= 1 && is_process_terminated`. Terminating games often keep driver-created threads or fail `OpenProcess` with `ERROR_ACCESS_DENIED`/`STATUS_PROCESS_IS_TERMINATING`. Now processes are omitted and their attribute/image path caches purged if `threads == Some(0)`, or `is_process_terminated(pid_u)`, or `!is_process_live(pid_u, start_epoch_s)` (with a 2-second grace period for newly spawned processes). Added `ImagePaths::remove(pid)` for immediate path eviction.
- 2026-09-18: WH_KEYBOARD_LL hotkey hook responsiveness and test isolation:
  - Optimized `WH_KEYBOARD_LL` low-level keyboard hook in `tm-platform::win::hotkey_hook`: elevated hook thread priority to `THREAD_PRIORITY_HIGHEST` to prevent scheduling delays from lagging system-wide keyboard input; decoupled window raising (`force_foreground`, `AttachThreadInput`, and UI signals) from the synchronous hook callback onto an asynchronous background worker thread (`tm-hotkey-worker`) driven by an auto-reset Win32 event; replaced mutexes with lock-free atomic debounce (`GetTickCount64`), and cached the IFEO replacement check with a 2-second TTL. The hook callback now returns in <1 microsecond with zero blocking operations.
  - Isolated minidump unit tests in `tm-platform::win::process_ops`: spawned an isolated dummy child process (`cmd.exe`) and waited for child initialization before dumping, preventing `MiniDumpWriteDump` from suspending test runner threads and deadlocking against parallel tests holding internal loader/MUI locks.
- 2026-09-18: Released v0.1.11 (tag on `cb495dd`, the bump commit). Followed `build.md` §Publishing path: `--check` + `--audit` green, stable clippy green, bump pushed on main, `build.py --all-targets` for all four archives (Windows x86_64/ARM64, Linux x86_64/ARM64), `sha256` beside each, `gh release create --latest` with the full sha as `--target`. Release covers: deliberate toast dismissal via an interactive close button (no auto-timeout or accidental text-click dismissal), service `SeDebugPrivilege` for elevated termination with enriched diagnostics, kernel memory composition visualization, dedicated/shared GPU metrics, measured startup impact, keyboard-navigable context menus, safe dialog keyboard defaults (Esc and unfocused Enter default to Cancel/Safe), and single-instance coordination improvements. Verified packaged archives: `taskman.exe` and `taskman-service.exe` in Windows zip, selfcheck verified.
- 2026-09-18: Process termination and core service privilege improvements:
  - Investigated and addressed `TerminateProcess: Zugriff verweigert (0x80070005)` on hung gaming processes (e.g. Alan Wake 2 hanging during GPU/D3D12/driver teardown). When a process is already shutting down, the Windows NT kernel (`PspTerminateProcess`) returns `STATUS_PROCESS_IS_TERMINATING` (0xC000010A), or if a driver created a system thread in the address space (`PsCreateSystemThread`), `PspTerminateThreadByPointer` returns `STATUS_ACCESS_DENIED` (0xC0000022). Both map via `RtlNtStatusToDosError` to Win32 `ERROR_ACCESS_DENIED` (0x80070005). User-mode termination (even running as `NT AUTHORITY\SYSTEM` with `SeDebugPrivilege`) is explicitly rejected by the Windows kernel to prevent BSODs and state corruption.
  - Updated `tm-platform::win::process_ops::terminate_handle`: if `TerminateProcess` fails, query `GetExitCodeProcess`; if `exit_code != STILL_ACTIVE`, the process has already terminated and the call succeeds (`Ok(())`). If still active, enriched the `ERROR_ACCESS_DENIED` error message with diagnostic context clarifying that the process is stuck in kernel/driver teardown or contains protected driver threads and cannot be killed from user mode even as SYSTEM.
  - Enabled `SeDebugPrivilege` at `tm-service` startup and in `open_destructive_process_verified`: SCM configured `SERVICE_CONFIG_REQUIRED_PRIVILEGES_INFO` with `SeDebugPrivilege`, but privileges start disabled in the token by default. Activating it at service launch and before destructive handle operations ensures worker threads have active debug privileges across sessions and restrictive ACLs.
- 2026-09-18: GPU and memory page improvements:
  - Removed superfluous shared memory graph on GPU page while retaining dedicated VRAM chart and shared memory bytes in page-bottom key-value statistics.
  - Software renderers (Microsoft Basic Render Driver) are filtered out during DXGI adapter enumeration in `tm-platform::win::gpu::adapters` whenever physical/hardware GPU devices are present; populated GPU driver version through DXGI `CheckInterfaceSupport(&IDXGIDevice::IID)`.
  - Fixed memory composition bar in-use calculation in `tm-app::tabs::performance::memory_composition_used`: `mem.used_bytes` is already `total - available` (active + modified) and does not contain standby cache; in-use memory is computed from `mem.total_bytes - (modified + standby + free)` so the legend below the bar reports true active memory rather than 0 B.
  - Interactive GPU metrics selection: `caption_dropdown` allocates an interactable arrow/dropdown target with hand cursor and hover highlighting; unified `menu::dropdown_menu` to handle both primary click toggle (anchored below the button) and secondary click open (at pointer), resolving a popup collision where concurrent dropdown and context menu calls caused left-clicks to immediately close and right-clicks to spawn two duplicate menus.
- 2026-09-18: Single-instance detection and prevention improvements: in `tm-platform::win::instance::handshake`, responsive windows (`window_responds == Some(true)`) are now treated as `Activation::Activated` with forced foregrounding, preventing duplicate instances when `ack` was consumed by a concurrent launch or delayed by modal tray menus / dialogs; re-checked `process_alive` on wait expiry to drop dead-instance timeout from 5.5s to 1.5s; added fast-path `activate_existing()` in `tm-app::main` prior to `redirect_to_installed_gui` to avoid binary SHA-256 hashing and SCM queries when an instance is already running; added `--instance-unresponsive` forwarding during auto-elevation so elevated child instances do not repeat a 5.5s probe against a known unresponsive instance.
- 2026-09-18: Integrated debug/developer tool discovery and local path overrides from `aufkrawall/llm-prompt-templates`: added `tools/discover-debug-tools.ps1` (with discovery for Windows SDK debuggers/signtool, MSVC, LLVM, Sysinternals, media/capture, Windows Performance Toolkit/ETW, and Rust verification tools) producing `%LOCALAPPDATA%\LLMDebugTools\debug-tool-manifest.json`, added `tool-paths.example.env`, updated `.gitignore` to keep local tool manifests/overrides uncommitted, and synchronized `AGENTS.md` and `llm-wiki/debug-tools*.md` tool resolution precedence.
- 2026-09-14: Released v0.1.10 (tag on `48030f0`, the bump commit). Followed `build.md` §Publishing path: `--check` + `--audit` green, stable clippy green, bump pushed on main, `build.py --all-targets` for all four archives (Windows x86_64/ARM64, Linux x86_64/ARM64), `sha256` beside each, `gh release create --latest` with the full sha as `--target`. Release covers: Private Working Set aligned per-process memory by default, explicit `Memory (active private working set)` column labeling and tooltips, comprehensive RAM metrics in Details (added `SharedWorkingSet` alongside `WorkingSet`, `PeakMemory`, and `Commit`), Process Properties statistics updates, real-time crash dump progress dialog with instant cancellation, and tray hotkey debounce fixes. Verified packaged archives: `taskman.exe` and `taskman-service.exe` pass `--selfcheck` in the Windows zip (reporting `version=0.1.10`).
- 2026-09-14: Explicit Private Working Set labeling and comprehensive RAM measurement columns: Details column picker and table header now explicitly title the default memory metric as `Memory (active private working set)` (`ColMemoryPrivateWs`), matching Windows Task Manager. Added `ColumnId::SharedWorkingSet` (`shared_working_set_bytes` = `working_set - mem_bytes`) to Details columns and properties dialog alongside `WorkingSet`, `PeakMemory`, and `Commit`. Header cells across `tablekit` now support localized tooltips (`TmColumn::with_tooltip`); the Processes and Users tab Memory header tooltip explains that it shows Private Working Set and directs users to Details for Working Set, Peak, Shared, and Commit metrics.
- 2026-09-14: Per-process memory metric aligned to Private Working Set by default: `SYSTEM_PROCESS_INFORMATION.WorkingSetPrivateSize` (offset 8) is extracted in `cpu_load.rs` and populated into `entry.mem_bytes` across Windows sampling, matching native Windows Task Manager (Processes & Details tabs). Total Working Set is preserved in `ProcessEntry::working_set_bytes` and exposed as an optional Details column `ColumnId::WorkingSet` and in process properties.
...
- 2026-09-11: Chart hover readouts now LOCK onto the sample they first named. A rolling chart slides a fresh sample under a motionless cursor on every tick, so the readout kept renaming its value and the spike being inspected was gone before it could be read. `widgets/chart.rs::pinned_sample` remembers a `Pin { at_ms, pointer }` in `ctx.data` under `<widget id>.with("readout-pin")` and resolves it back to an index through the new `nearest_at()`; because the marker is drawn from that index it travels left with the sample instead of standing still over changing data, and the existing age line ("12 s ago") counts up so the lock reads as deliberate. Released when the pointer moves more than `PIN_SLACK` (1 px, so sub-pixel jitter does not count), when the pinned sample rolls out of the window, and when the chart stops being hovered (`hover_pos` clears it). Charts with no timestamps, or fewer timestamps than samples, never take a pin and stay on the even-spacing fallback.

- 2026-09-11: Graphs compressed their own history instead of scrolling it. `chart_multi` derived the x axis from the samples themselves (`t0 = first`, `span = last - first`), which pins the oldest sample to the left edge however little history exists: a half-filled window drew 30 s of data across a caption reading 60 s, and each new sample squeezed the older ones slightly further left rather than moving the whole curve. Visible as "the left side is static, only the right edge updates" — for the first `graph_seconds` after launch, and for the full duration again after widening the window setting. `core_chart` had the same defect in its even-spacing form (`i / (n - 1)` over a growing `n`). Both now take `chart::TimeAxis { stamps, window_ms }`, built per page beside `timestamps(win)` from `settings.graph_seconds`: the newest sample sits at the right edge, every other sample is placed by its real age, and the part of the window with no data yet stays empty the way Task Manager's does. Pinned by `the_axis_spans_the_configured_window_not_the_samples_it_has`, which reads the painted polyline back out of the frame.

- 2026-09-11: Released v0.1.6 (tag on `4840c52`, the bump commit). Same `build.md` §Publishing path as v0.1.5 and no new surprises: `--check` + `--audit` green, bump pushed on main, `build.py --all-targets` for all four archives, `sha256` beside each, `gh release create --latest` with the FULL sha as `--target`. One format note worth keeping: `sha256sum` writes `<hash>  *<file>` in binary mode, and the published `.sha256` files are normalised to the documented `<hash>  <file>` so `sha256sum -c` works from the download directory. Verified by re-downloading all four: checksums match, the Windows zip carries `taskman.exe` + `taskman-service.exe` and passes `--selfcheck --mock`, and the Linux x86_64 binary passes a real `--selfcheck` under WSL.

- 2026-09-12: Released v0.1.7 (tag on `c235a64`, the bump commit). Routine run of the `build.md` §Publishing path, nothing new: `--check` + `--audit` green, bump pushed on main, `build.py --all-targets --require-all-targets` for all four archives, `sha256` beside each, `gh release create --latest` with the full sha as `--target`. Small release — the only user-visible change since v0.1.6 is the charts' left-edge lead-in plus the 62x40 → 74x48 mini graphs; the rest is the vendored-winit `*.png` ignore fix and the `array::from_fn` clippy fix. Verified by re-downloading the x86_64 pair: `sha256sum -c` OK, the Windows zip carries `taskman.exe` + `taskman-service.exe` and passes `--selfcheck` on both the mock and the real backend (reporting `version=0.1.7`), and the Linux x86_64 binary passes a real `--selfcheck` under WSL. The two ARM64 artifacts remain unrunnable on this host, so they are build-verified only.

- 2026-09-12: The GUI randomly stopped recognising a healthy core service, and the cause was that the broker was dying. `connect_pipe` mapped `ERROR_NO_DATA` to `Err` and the accept loop propagated it out of `run_broker`, so any client that opened the pipe and dropped the handle before the server reached `ConnectNamedPipe` terminated the LocalSystem service; SCM recovery restarted it 5 / 15 / 60 s later. Reproduced from an ordinary user session: ~17 connect-and-close opens killed it (System event 7024, service-specific error 1, then 7031 + restart). A second defect fed it — the accept loop re-armed its listener only AFTER resolving the accepted client's image, leaving a window with no listening instance, so a single-threaded client saw `ERROR_PIPE_BUSY` on roughly half its opens and the client's single `WaitNamedPipeW` retry regularly lost the follow-up race too. Fixes: `connect_pipe` returns `AcceptOutcome` and a vanished client spends the instance without failing; no per-connection error terminates the broker (16 consecutive failures still do, so a dead listener stays visible); the listener is re-armed before identity resolution (`PIPE_INSTANCE_CAP` 19 → 20); the client retries busy until a 1 s deadline; `service_state` no longer calls an unanswered pipe an authentication failure; and the sampler tolerates 3 consecutive unreachable ticks before demoting a working broker source for 30 s. Pinned by `a_client_that_closes_before_accept_is_not_a_broker_failure`. Note the fix only takes effect once the installed service binary is replaced — Advanced settings → Repair.

- 2026-09-12: Verified the broker fix against the repaired install and closed the follow-up it exposed. Before: ~17 connect-and-close opens killed the service. After: 1400 of them left it RUNNING, and a client mirroring the product's retry loop got 1050/1050 opens with a mean of 0.1-0.34 ms and a WORST case of 2.5 ms even with four threads hammering — so `PIPE_BUSY_RETRY` (1 s) has ~400x headroom and only bites on a genuinely wedged broker. Careful with raw busy counts as a metric: a tight C# loop doing nothing but CreateFile/CloseHandle outruns any single-threaded acceptor and reports ~99% `ERROR_PIPE_BUSY` on the FIXED server, which says nothing about the product path — measure the retry loop's outcome and latency, not the first-attempt rate. Separately, "the GUI picks up a newly installed service but takes several seconds" was the sampler backing off the BROKER and the LOCAL ETW trace together: one `NET_SOURCE_RETRY` (30 s) timer gated both, so a service installed from Advanced settings could wait most of that window. The broker is now re-checked every tick while the source is `Unavailable` (an absent one costs a single failing `CreateFileW`) and the 30 s timer throttles only the local ETW start. The residual delay is inherent and stays: a rate needs two cumulative samples and the first disk window is discarded below `MIN_WINDOW_MS`, so the columns fill about two ticks after the broker answers.

- 2026-09-12: Per-process "Disk active time" was hypersensitive - rows flapping between 0 % and 100 % - because it was a share of the window's disk service time, NOT a share of time. `apply_process_disk` computed `proc_service_time / total_service_time`, so on a nearly idle disk one I/O request made its issuer 100 % of all the disk work that happened: a true statement about a denominator carrying no information. Measured on this machine with repeated `--selfcheck` runs of the installed GUI: `disk_busiest_active_pct=0.00 %` with `process_disk_busiest_pct=91.18 %`, and `0.00 %` with `41.94 %` - a row reading 91 % under a header reading 0 %. The old design was deliberate and documented in `Aggregates::strings` ("the disks are 43 % busy and this process accounts for 100 % of that"), but it has no guard on the denominator and the row is never readable on its own. Rows are now scaled by the machine total (`share x machine_active_pct`), so the column means what its label says - the percentage of WALL TIME the disks were busy for that process - rows sum to the header instead of needing it as context, and the noise collapses in proportion to how little the disks did. An unmeasured machine total leaves rows unknown ("-") rather than fabricating one. The scaling lives in the pure `sampler::disk_active_pct` so `disk_active_time_is_wall_time_not_a_share_of_an_idle_disk` can pin it on the real measured numbers, including that no row ever exceeds the header. Note the dev tree cannot verify this end to end: an unelevated dev-tree binary is a foreign client the broker rejects, so `--selfcheck` reports `process_disk_readings=0` (correctly unknown) - the installed GUI is needed.

- 2026-09-12: The Disk active time column flashed "-" for one tick after restoring a minimized window. `apply_process_disk` rejects a window shorter than `MIN_WINDOW_MS` (100 ms) and returned early, which leaves EVERY row `None` - the whole column blanks at once, which is what makes it visible. Two things produce a short window: the sampler thread being starved while the window is restored, so the next engine tick lands milliseconds after the last one; and `probe_disk_source`, which deliberately threw its sample away. Throwing it away is not free the way it is for network, because the disk answer DRAINS its window: the discarded probe left the next drain covering only the microseconds since it, guaranteeing a rejected window on every probe tick. `probe_disk_source` now returns `(DiskSource, Option<ProcessDiskSample>)` like `probe_net_source` does, and a rejected window carries the previous tick's values forward (`Sampler::last_proc_disk`, pruned to live pids, cleared whenever the source is given up so nothing is inherited across a gap of unknown length) instead of reporting unknown. The carry is identity-checked: a recycled PID gets unknown, never the dead process's numbers. Same convention the CPU path already used ("when a window is too short to be trustworthy we keep the previous sample"). Pinned by `a_window_too_short_to_measure_carries_the_last_reading_forward`.

- 2026-09-12: Released v0.1.8 (tag on `1c5e6a9`, the bump commit). Followed `build.md` §Publishing path: `--check` + `--audit` green, stable clippy green, bump pushed on main, `build.py --all-targets --require-all-targets` for all four archives, `sha256` beside each, `gh release create --latest` with the full sha as `--target`. Release covers: LocalSystem broker service stability (vanished clients no longer terminate broker, pipe listener race eliminated, faster retry/broker detection), disk active time scaled by machine total to wall time with short-window carry-forward, and table scroll anchoring majority displacement so CPU-sorted lists no longer drift downwards. Verified packaged archives: `taskman.exe` and `taskman-service.exe` pass `--selfcheck` in the Windows zip (reporting `version=0.1.8`), and the Linux x86_64 binary passes real `--selfcheck` under WSL.

- 2026-09-14: Released v0.1.9 (tag on `491427e`, the bump commit). Followed `build.md` §Publishing path: `--check` + `--audit` green, stable clippy green, bump pushed on main, `build.py --all-targets` for all four archives, `sha256` beside each, `gh release create --latest` with the full sha as `--target`. Release covers: configurable process dump types (Minimal, Limited, Normal, Full) via context submenu matching System Informer / Process Hacker, and WH_KEYBOARD_LL low-level keyboard hook interception of Ctrl+Shift+Esc in fullscreen games with forced foreground window activation. Verified packaged archives: `taskman.exe` and `taskman-service.exe` pass `--selfcheck` in the Windows zip (reporting `version=0.1.9`), and the Linux x86_64 binary passes real `--selfcheck` under WSL.

- 2026-09-22: Source-quality audit pass (user-requested "lowest quality aspects" review, then implement the recommendations). Four behavioral fixes: (1) `set_efficiency_mode_batch` returned `Ok(())` unconditionally and toasted success for an all-refused batch — the count/error pattern now lives once in `action_executor::apply_batch` (count successes, `Err(first_error)` only when ZERO succeeded), shared by the efficiency, priority and affinity batches, pinned by `apply_batch_*` unit tests; (2) the Services tab spawned a raw `tm-svc-fetch` thread instead of using the executor, so a failed spawn left `services_fetch` claimed forever ("Gathering data" with no recovery) — it now runs through `run_quiet` with the same queue-full/None fallbacks as Startup/Users; (3) eleven `settings.save()` call sites wrote `config.ini` synchronously on the UI thread, bypassing the coalescing `SettingsWriter` — all now go through `app.save_settings()`, and `Settings::save` (the blocking entry point) is deleted rather than left as a trap; (4) `hotkey_hook`'s `HOOK_CALLBACK.lock().unwrap()` was the last production raw lock — switched to `tm_core::sync::lock` (poison recovery; a poison panic here would abort the process carrying desktop keystrokes). Structural/dedup: `tablekit` gained `cmp_ignore_case` (iterator-based, zero allocation per comparison — every tab used to build two `String`s per `sort_by` comparison), `directed`, and `TmTable::apply_auto_fit`, adopted across all seven table tabs; the module-wide `#[allow(dead_code)]` on `app_ui::original` is gone (the shadowed duplicate `original::tab_header` it existed for was deleted), and the three `dispatch_core_service_*` functions moved from the chrome file into `action_executor.rs` where the lane they dispatch onto lives. New tests: i18n de/en placeholder-parity over every key (macro now emits `ALL_KEYS`), Services sort contracts (case-insensitive, pid-less last in either direction, native status rank), executor queue/toast behavior. `SAFETY:` comments added to the riskiest unsafe sites (FreeLibrary transmute, sampler termination check, hook struct deref, broker handle/SID transfers). Naming: `tabs/apphistory.rs` → `tabs/app_history.rs` (persisted string keys `"apphistory"` deliberately unchanged — config.ini compat). Dead workspace `windows` entry now the single version source (`workspace = true` in tm-app/tm-platform). Deliberately NOT done this pass (assessed, not worth the churn/risk yet): splitting the god-files (`details.rs` 4.8k, `processes.rs` 4.4k, `sampler.rs::sample_inner` ~630 lines) — mechanical but a huge diff; worth doing as a dedicated follow-up.
- 2026-09-24: Targeted source-quality follow-up. `stop_service_for_upgrade`
  now stops and waits through SCM status rather than requiring `SYNCHRONIZE`
  on the LocalSystem process; live upgrade/rollback still needs disposable-VM
  validation. Core-service repair/switch controls clear in-flight state on
  errors. The sampler no longer promotes same-name candidate paths into a
  process identity (and therefore into saved scheduling-rule matching), nor
  infers SYSTEM/SID/elevation from Session 0 or a familiar name alone. App
  History preserves a slow deferred load, merges observations after it
  arrives, skips first-sighting rate overcount, and tracks non-system
  processes on Unix where App classification has no window source. Broker
  network/disk calls run on one
  bounded client worker so a silent service cannot freeze sampling; oversized
  telemetry responses report unknown rather than partial rows as zeros.
  The service-host token unit test now asserts only the current user's readable
  token because this unelevated Windows host denied TOKEN_QUERY for all 84-85
  service hosts; the SCM catalog has its own account coverage. The full gate
  exposed `cargo-zigbuild` installed without `zig`; `build.py` now selects its
  musl fallback in that case. The automated gate passed. Installed-service
  upgrade/rollback and elevated ETW behavior still require disposable-VM
  testing.
