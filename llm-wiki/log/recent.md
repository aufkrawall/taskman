- 2026-09-18: GPU and memory page improvements:
  ...
  - Interactive GPU metrics selection: `caption_dropdown` allocates an interactable arrow/dropdown target with hand cursor and hover highlighting; wired up `gpu_graph_dropdown_menu` via `menu::dropdown_menu` so left-clicking the arrow icon drops down the engine selector matching right-click context menu behavior.
