- 2026-09-18: Ghost/zombie process eviction and hotkey hook responsiveness:
  - Addressed lingering ghost processes (e.g. crashed game processes like Alan Wake 2 continuing to show until TaskMan restart): on Windows NT, external `PROCESS_VM_READ` handles held by `sysinfo` prevent the kernel from completing address-space rundown when an application crashes, creating a circular handle dependency where the process cannot leave Toolhelp snapshots while `sysinfo` holds the handle. Added `is_process_terminated` in `tm-platform::win::sampler` using `PROCESS_SYNCHRONIZE` (`WaitForSingleObject(h, 0) == WAIT_OBJECT_0` and exit code check) to detect exited processes, omit them immediately from snapshots, and recreate `self.sys = System::new()` upon dead process detection to evict all handles and unblock kernel teardown.
